...
- **Source of truth:** `.claude/agents/legacy-stabilization-auditor.md` (frontmatter + body).
- **Codex prompt:** the same file with its YAML frontmatter stripped (`install.sh` does this). Codex has no tool-restricted subagents, so read-only is enforced by the instruction text in the body, not by tooling.

## Rules for editing

- Edit the agent's behavior **only** in `.claude/agents/legacy-stabilization-auditor.md`. Never hand-edit an installed copy under `~/.claude` or `~/.codex`.
...
```
.claude/agents/legacy-stabilization-auditor.md   canonical agent (Claude subagent)
